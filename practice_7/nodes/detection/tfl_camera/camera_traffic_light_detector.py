...
class CameraTrafficLightDetector:
    ...
    def local_path_callback(self, local_path_msg):
        ...
        if len(local_path_msg.waypoints) > 0:
            local_path_waypoints = np.array([(w.pose.pose.position.x, w.pose.pose.position.y) for w in local_path_msg.waypoints])
            local_path_linestring = LineString(local_path_waypoints)

            for id, stopline in self.tfl_stoplines.items():
    ...
    def camera_image_callback(self, camera_image_msg):
        ...
        if len(stoplines_on_path) > 0:
            ...
            if len(rois) > 0:
                ...
                classes = np.argmax(np.array(predictions), axis=1)
                scores = np.max(np.array(predictions), axis=1)
        ...
        for stoplineId in stoplines_on_path:
            for plId, bulbs in self.signals[stoplineId].items():
                ...
                for _, _, x, y, z in bulbs:
                    ...
                    # transform point in the camera frame to pixel coordinates
    ...
    def create_roi_images(self, image, rois):
        ...
        for _, _, min_u, max_u, min_v, max_v in rois:
            roi = np.array(image)[min_v: max_v, min_u:max_u]
